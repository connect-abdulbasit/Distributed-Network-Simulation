...
# Script to start all services with 10 instances each
...
echo ""
echo "Auth Service Instances (${INSTANCES} total):"
for i in $(seq 1 $INSTANCES); do
    PORT=$((3000 + i))
    PID=${AUTH_PIDS[$((i-1))]}
...
echo ""
echo "Data Service Instances (${INSTANCES} total):"
for i in $(seq 1 $INSTANCES); do
    PORT=$((4001 + i))
    PID=${DATA_PIDS[$((i-1))]}
    echo "  - Instance ${i} (PID $PID): http://localhost:${PORT}"
...
echo ""
echo "Compute Service Instances (${INSTANCES} total):"
for i in $(seq 1 $INSTANCES); do
    PORT=$((5001 + i))
