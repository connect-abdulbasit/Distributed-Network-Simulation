...
// Helper function to get health endpoint (fallback)
...
async function makeRequest() {
  ...
  try {
    const url = `${LOAD_BALANCER_URL}${ENDPOINT}`;
    let response;
    
    // Use POST for registration/login endpoints, GET for others
...
function displayStats() {
  ...
  console.log(`  Target:        ${colors.magenta}${LOAD_BALANCER_URL}${ENDPOINT}${colors.reset}`);
...
async function runLoadTest() {
  ...
  const interval = 1000 / REQUESTS_PER_SECOND; // milliseconds between requests
  ...
  // Wait until end time, then wait for pending requests
...
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log(`
${colors.bright}Load Testing Script${colors.reset}
...
Examples:
  ...
  # Test auth endpoints (requires valid data)
  ENDPOINT=/api/auth/login node load-test.js

  # Test with custom load balancer
